...
pub fn load_model() -> Result<(QMixFormer, Tokenizer)> {
    ...
    let vb = candle_transformers::quantized_var_builder::VarBuilder::from_gguf(&weights_filename, &Device::Cpu)?;
...
pub async fn answer_with_context(query: &str, references: Vec<Content>) -> Result<String> {
    ...
    // Create the prompt
    let prompt = format!("<|im_start|>system\nAs an angry medieval archery trainer named Borland. We are in a medieval training facility, practising fighting. You are my instructor. Your answer should be very concise and to the point. Do not use more than 120 characters in an answer. Do not repeat question or references.<|im_end|>\n<|im_start|>user\nquestion: \"{question}\"\nreferences: \"{context}\"\n<|im_end|>\n<|im_start|>assistant\n", context=context, question=query);
